...
<img src = "https://github.com/northwest-syria-cash-working-group/multisector_4Ws_review/raw/main/NWS-CWG%20logo.PNG"  width="80%"/>
...
**`r paste0(round(com %>% filter(project_status %in% c("Completed", "Ongoing") & activity != "ISIMM") %>% mutate(cva = ifelse(delivery_modality %in% c("Cash", "Voucher"), beneficiaries, 0)) %>%  {sum(.$cva, na.rm = TRUE)} / 1000000, digits = 2), " million")`** beneficiary frequencies reached by CVA. This is **`r cva_pc`%** of all beneficiary frequencies. 
...
com %>% 
  ...
  footnote(i = 1, j = 6, part = "header", ref_symbols = "1",
           as_paragraph("Average number of clusters present per community"))
