...
read_sheet("https://docs.google.com/spreadsheets/d/1KGqt-3YDh2k8qNCksOJAndRPO56Gq6pGs6esEQtIFIw/edit#gid=1890981115", 
  ...
  write_csv("./data/fsl_tracker.csv")
```
...
erl <- read_excel("./data/ocha_unedited/2023-02Feb_ERL_4Ws_NWS - WoS - V03 -WoS Validated.xlsx", 
  ...
  mutate(total_usd = transfer_value * beneficiaries)
...
com <- rbind(
  ...
  mutate(activity = 
           case_when(str_detect(activity, "Activity 2.01 Financial") ~ "Grants, seed funds, loans",
                     ...
                     activity == "multipurpose cash" ~ "Multipurpose cash", 
