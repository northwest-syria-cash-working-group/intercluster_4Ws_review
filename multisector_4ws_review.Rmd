---
title: "DRAFT Multisector coverage and gaps review"
...
Please use the table on contents on the right to navigate through this document. This document distinguishes between **beneficiaries** (unique individuals reached) and **beneficiary frequencies** (the total number of times an individual has received assistance, inclusive of double counting). 
...
com %>% 
  ...
  flextable() %>% 
  theme_zebra() %>% 
  set_caption("4Ws datasets provided to OCHA, 19 May 2023") %>% 
  set_table_properties(layout = "autofit", width = .5)
```

<br>

*As a note, data was extracted on May 19 2023 from the FSL live tracker.* 
...
```{r}
com %>% filter(activity != "ISIMM" & project_status %in% c("Completed", "Ongoing")) %>% 
  filter(admin4pcode == "C4226") %>% 
  group_by(activity) %>% 
  summarise(beneficiaries = sum(beneficiaries, na.rm = TRUE)) %>% 
...
Allocation is decidedly skewed: the **top 20** communities have received **`r top_20 %>% pull(pc_ben)`%** of all beneficiary frequencies. Humanitarian partners have completed activities or are implementing activities in **`r com %>% filter(project_status %in% c("Completed", "Ongoing") & activity %out% c("ISIMM")) %>% {n_distinct(.$admin4pcode)}`** communities. 
...
com %>% 
  ...
  annotate(geom = "rect",
           xmin = 0.0028814438, xmax = 0.08, 
            ymin = 0.01268678, ymax = 0.08, 
            colour = "goldenrod", alpha = 0) + 
  geom_point(aes(size = beneficiaries), 
             alpha = .7) +
  scale_size_continuous(breaks = c(100, 1000, 10000, 100000, 300000, 750000), 
...
Each square is a single community. The figure in each square shows the percentage of cash beneficiaries reached in that community. 
...
com %>% 
  ...
  labs(x = "Number of clusters", 
       y = "Number of communities", 
       title = "251 communities only have one cluster present")
```
...
* But also be aware of whilst MPC can help resolve a large range of issues related to low consumption, it cannot resolve systemic such as a lack of housing, land or property rights nor can it fix a sewage system, nor can it engender behaviour change. 
...
* Coordinate with FSL to ensure that overlap is limited between FSL and MPC. Food baskets should not be provided to those who have received MPC. Use this opportunity to broaden your agency footprint. The single largest household expenditure, according to the MSNA, is food. 
